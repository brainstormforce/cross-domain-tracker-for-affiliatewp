...
final class Affiliate_WP_Track_External_Visits {
	...
	public function cdtawp_connection_section_callback() {
		...
		$affiliate_api_link = $affiliate_api_link ? $affiliate_api_link : '';

		echo "<p id='cdtawp_auth_desc'> To enable tracking of site visits, you need to authenticate with parent website <i>( " . esc_url( $affiliate_api_link ) . " )</i> where AffiliateWP is installed.  <br/>Please read <a target='_blank' href='https://docs.affiliatewp.com/article/1453-rest-api-authentication'>this article</a> to obtain API keys from parent website.";
	...
	public function callback_input_text( $args ) {
		...
		$value = isset( $options[ $args['name'] ] ) ? $options[ $args['name'] ] : '';
		?>
		<input style="width: 30%" type="text" id="<?php echo esc_attr( $args['id'] ); ?>" name="<?php echo CDTAWP_SETTINGS_GROUP; ?>[<?php echo $args['name']; ?>]" value="<?php echo sanitize_text_field( $value ); ?>"/>
	...
	public function callback_input_checkbox( $args ) {
		...
		?>
		<input <?php echo $value ? 'checked' : ''; ?> type="checkbox" id="<?php echo esc_attr( $args['id'] ); ?>" name="<?php echo CDTAWP_SETTINGS_GROUP; ?>[<?php echo $args['name']; ?>]" value="<?php echo $value; ?>"/>
